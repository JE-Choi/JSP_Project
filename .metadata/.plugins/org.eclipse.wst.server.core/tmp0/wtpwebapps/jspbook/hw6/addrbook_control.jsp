...
<jsp:useBean id="ab" class="jspbook.hw6.AddrBean"/> <!-- 데이터베이스 연동부분 --> 
<jsp:useBean id="addrbook" class="jspbook.hw6.AddrBook"/>
<!-- request보낸 jsp에서 인자로 넘겨 받은 걸로 addrbook초기화 -->
...
<% 
	...
	// 주문 목록에서 요청한 사용자 정보
		...
		request.setAttribute("datas_addr", datas);
		pageContext.forward("order_oid.jsp");
	}
	//[고객] 주소록 등록 요청인 경우
		else if(action.equals("c_insert")) {
			if(ab.insertDB(addrbook)){// insert가 성공이 되었다면?
				// addrbook_list.jsp를 호출을 해야하지만, 처리는 control이 하는 것이 맞기에 간접적으로 control을 redirect함.
				session.setAttribute("id", addrbook.getAb_id());
	...
	// 주소록 수정 페이지 요청인 경우
		...
		// 	0527 if (request.getParameter("ab_pwd").equals(addrbook.getAb_pwd())){
			// 0527  이부분을 관리자가 아닐때는 수정하면 안되니 1234로 해야 합니다. 
		if (request.getParameter("ab_pwd").equals("1234")){
			// 수정 작업
